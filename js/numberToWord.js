...
//function isNumeric() {
//        var elem = document.niw.num.value; if (elem != "") {
//            var numericExpression = /^[0-9]+$/; if (elem.match(numericExpression)) {
//                return true;
//            } else {
//                alert("Please Enter Only Number "); document.niw.num.value = "";
//                return false;
//            }
//        }
//    }
function numinwrd(number) {
    var numbr = number; var str = new String(numbr)
    ...
    for (i = 0; i < numlen; i++) {
        switch (i) {
            case 0: if ((rev[i] == 0) || (rev[i + 1] == 1)) {
                word[j] = '';
            } else { word[j] = once[rev[i]]; } word[j] = word[j]; break; case 1: abovetens();
                break; case 2: if (rev[i] == 0) { word[j] = ''; } else if ((rev[i - 1] == 0) || (rev[i - 2] == 0)) {
                    word[j] = once[rev[i]] + " Hundred ";
                } else { word[j] = once[rev[i]] + " Hundred and "; } break; case 3: if (rev[i] == 0 || rev[i + 1] == 1) {
                    word[j] = '';
            ...
            case 5: if ((rev[i] == 0) || (rev[i + 1] == 1)) {
                word[j] = '';
            } else { word[j] = once[rev[i]]; } word[j] = word[j] + " Lakh "; break;
            case 6: abovetens(); break; case 7: if ((rev[i] == 0) || (rev[i + 1] == 1)) {
                word[j] = '';
